...
class Perceptron():
    def __init__(self, Nodes, parallel = False):
    ...
    def set_train(self, training_input, training_output, training_iterations):
        for it in range(training_iterations):
            key_t = 't{}'.format(it)
            self.guessed[key_t] = 0
            self.train(training_input, training_output, training_iterations)
